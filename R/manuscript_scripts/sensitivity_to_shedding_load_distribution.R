...
influenza_data <- read.csv("app/data/ww_loads.csv") %>%
    filter(wwtp == "ARA Werdhölzli", observation_units == "gc_per_day_norm", influenza_type == "IAV")
...
for (i in 1:length(distributions)) {
    ...
    WW_estimates <- get_block_bootstrapped_estimate(
        incidence_data = influenza_data$observation,
...
# Plot results
ggplot(
    data = WW_estimates_all %>% filter(delay_type != "Carrat 2008 MLE"),
