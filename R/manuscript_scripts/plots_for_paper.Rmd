...
ww_re_scale_sensitivity <- read.csv("data/data_used_in_manuscript/ww_re_scaling_sensitivity.csv")
...
# tmp <- data_to_plot %>% filter(observation_type == "Lugano", influenza_type == "IAV", data_type == "Cases")
...
ggsave("figures/deconvolved_incidence_comparision_focus.png", width = 6.5, height = 4.5, units = "in")
```
...
ggsave("figures/re_comparision_focus.png", width = 6.5, height = 3.25, units = "in")
```
...
delay_dist_incidence_plot <- ggplot(
  data = all_re_clean %>% 
    filter(data_type == "Wastewater", observation_type != "Basel", measuring_period == "2021/22")) +
  geom_hline(aes(yintercept = 1), color = "darkgray") +
...
ww_loads %>%
  ...
  group_by(wwtp, measurement_type) %>%
  summarize(n_detect_days = sum(detected), total_days = n(), .groups = "drop") %>%
