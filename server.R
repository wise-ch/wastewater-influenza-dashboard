...
function(input, output, session) {
    ...
    output$hover_info_raw <- renderUI({
        ...
        wellPanel(
            style = style,
            p(HTML(paste0("<i>", point$date, "</i>", "<br/>",
                          "<b> Gene copies</b> (x10<sup>12</sup>): ", round(point$n1, 2), "<br/>",
    ...
    output$link <- renderUI({
        ...
        lod_loq <- p('**Limit of detection (LOD) represents concentration levels at which SARS-CoV-2 can be reliably detected, 
                    while limit of quantification (LOQ) represents concentration levels with prespecified precision of detection.
