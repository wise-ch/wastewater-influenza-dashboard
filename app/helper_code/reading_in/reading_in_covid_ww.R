...
load_covid_ww_data <- function() {
  ...
  ww_data_sars_cov_2 <- ww_data %>%
    mutate(protocol = "v3.1") %>%
...
ww_read_in <- function(data_url, region) {
  raw_data <- read_delim(data_url,
    delim = ";",
    col_names = c(
      "date", "n1", "n1_smooth", "cases",
  ...
  new_case_data <- raw_data %>%
    select(date, cases, cases_smooth) %>%
    complete(date = seq.Date(min(date), max(date), by = "days")) %>% #
    mutate(region = region)

  new_ww_data <- raw_data %>%
    filter(!is.na(n1)) %>%
