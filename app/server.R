...
function(input, output, session) {
  ...
  observeEvent(input$pathogen, {
    ...
    output$data_type <- renderUI({
      checkboxGroupInput(
        ...
        choices = options_enabled,
        selected = "Wastewater"
      ) # this assumes wastewater is an available data type for all pathogens
    })
  ...
  observeEvent(input$region, {
    ...
    date_range <- c(
      range((plotDataWW %>% filter(region == input$region) %>%
        select(date))[["date"]])[1],
      Sys.Date()
    )
  ...
  output$case_plots <- renderPlot({
    ...
    case <- case_plotter(
      data = plotDataObs, canton = input$region,
  ...
  output$hover_info_case <- renderUI({
    ...
    point <- nearPoints(select_data,
      hover_case,
      threshold = 4, maxpoints = 1, addDist = TRUE
    )
    if (nrow(point) == 0) {
      return(NULL)
    }
  ...
  output$re_plots <- renderPlot({
    req(input$pathogen)
    re <- re_plotter(
      data = plotDataRe, source = input$data_type, canton = input$region,
  ...
  output$zh_plot <- renderPlot({
    plot_Re(
      re_var %>% filter(source %in% input$variant_source), "ZH", input$variant,
      "source", c("Wastewater (NGS)", "Catchment cases"), c("Wastewater (NGS)", "Catchment cases")
    )
  })
  output$sg_plot <- renderPlot({
    plot_Re(
      re_var %>% filter(source %in% input$variant_source), "SG",
      input$variant, "source", c("Wastewater (NGS)", "Catchment cases"), c("Wastewater (NGS)", "Catchment cases")
    )
  })
  output$gr_plot <- renderPlot({
    plot_Re(
      re_var %>% filter(source %in% input$variant_source), "GR",
      input$variant, "source", c("Wastewater (NGS)", "Catchment cases"), c("Wastewater (NGS)", "Catchment cases")
    )
  })
  output$fr_plot <- renderPlot({
    plot_Re(
      re_var %>% filter(source %in% input$variant_source), "FR",
      input$variant, "source", c("Wastewater (NGS)", "Catchment cases"), c("Wastewater (NGS)", "Catchment cases")
    )
  })
  output$ti_plot <- renderPlot({
    plot_Re(
      re_var %>% filter(source %in% input$variant_source), "TI",
      input$variant, "source", c("Wastewater (NGS)", "Catchment cases"), c("Wastewater (NGS)", "Catchment cases")
    )
  })
  output$ge_plot <- renderPlot({
    plot_Re(
      re_var %>% filter(source %in% input$variant_source), "GE",
      input$variant, "source", c("Wastewater (NGS)", "Catchment cases"), c("Wastewater (NGS)", "Catchment cases")
    )
  })
  output$hover_info_variant_zh <- renderUI({
    ...
    variant_data <- re_var %>%
      filter(source %in% input$variant_source) %>%
  ...
  observeEvent(input$pathogen_switzerland, {
    ...
    output$region_switzerland_disabled <- renderUI({
      disabled(checkboxGroupInput(
        inputId = "region_switzerland_disabled",
        label = NULL,
        choices = options_disabled
      ))
    })
  ...
  output$rww_plots <- renderPlot({
    rww <- canton_plotter(
      source = "Wastewater", canton = input$region_switzerland,
  ...
  output$hover_info_rww <- renderUI({
    ...
    select_data <- plotDataRe %>%
      filter(region %in% input$region_switzerland, pathogen_type == input$pathogen_switzerland) %>%
  ...
  output$rcc_plots <- renderPlot({
    rcc <- canton_plotter(
      canton = input$region_switzerland, source = "Confirmed (Catchment)", pathogen = input$pathogen_switzerland,
  ...
  output$link <- renderUI({
    ...
    exclude_lod <- p(HTML(paste0(
      strong("NB: "), i18n$t("Wastewater measurements from 02.2021 to 07.03.2021 for "), canton_to_catchment[[input$region]],
    ...
    if (input$region == "GR") {
      HTML(paste(link, exclude_lod, sep = ""))
    } else if (input$region == "FR") {
      HTML(paste(link, laupen_2cantons, sep = ""))
    } else if (input$region == "TI") {
      HTML(paste(link, exclude_lod, sep = ""))
    } else {
      HTML(paste(link, sep = ""))
    }
