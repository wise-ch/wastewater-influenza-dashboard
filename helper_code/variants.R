...
variants <- c('B.1.1.7', 'B.1.617.2', 'BA.1', 'BA.2')
...
variant_ww <- new_df %>% dplyr::rename(region = 'location') %>%
...
for (i in 1:length(variants)) {
  variant_url <- paste0('https://lapis.cov-spectrum.org/gisaid/v1/sample/aggregated?country=Switzerland&pangoLineage=',variants[i],'&fields=date&dataFormat=csv')
  variant_cases <- read_delim(variant_url)
  total_url <- 'https://lapis.cov-spectrum.org/gisaid/v1/sample/aggregated?country=Switzerland&fields=date&dataFormat=csv'
...
clinical_variant_data <- case_data %>% left_join(variant_case_proportions, by = 'date') %>%
  group_by(region, variant) %>%
  complete(date = seq.Date(min(date), max(date), by = 'days')) %>% # so we lose out on the newer cases..
...
clinical_variant_data <- clinical_variant_data %>% filter(region %in% regions) %>% filter(variant %in% vars)
